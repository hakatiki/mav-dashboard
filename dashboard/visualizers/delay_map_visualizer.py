...
class DelayAwareRouteMap:
    ...
    def create_delay_legend(self, map_obj):
        ...
        <div style="position: fixed; 
                    ...
                    box-shadow: 0 0 15px rgba(0,0,0,0.2);">
                         <h4 style="margin: 0 0 10px 0; text-align: center;">🚂 Késések</h4>
             <div style="margin: 3px 0;">
                 <span style="display: inline-block; width: 20px; height: 15px; 
                             background-color: {no_delay}; border: 1px solid #ccc; margin-right: 8px;"></span>
                 Időben (&lt; 2 perc)
             </div>
             <div style="margin: 3px 0;">
                 <span style="display: inline-block; width: 20px; height: 15px; 
                             background-color: {minor_delay}; border: 1px solid #ccc; margin-right: 8px;"></span>
                 Kis késés (2-10 perc)
             </div>
             <div style="margin: 3px 0;">
                 <span style="display: inline-block; width: 20px; height: 15px; 
                             background-color: {moderate_delay}; border: 1px solid #ccc; margin-right: 8px;"></span>
                 Közepes (10-20 perc)
             </div>
             <div style="margin: 3px 0;">
                 <span style="display: inline-block; width: 20px; height: 15px; 
                             background-color: {severe_delay}; border: 1px solid #ccc; margin-right: 8px;"></span>
                 Súlyos (20+ perc)
             </div>
             <div style="margin: 3px 0;">
                 <span style="display: inline-block; width: 20px; height: 15px; 
                             background-color: {no_data}; border: 1px solid #ccc; margin-right: 8px;"></span>
                 Nincs adat
             </div>
        </div>
        '''.format(**self.colors)
    ...
    def add_delay_routes(self, map_obj, routes, enriched_segments):
        ...
        print("🎨 Creating delay-aware route visualization...")
        
        # Create segment lookup for delay information
        segment_delays = {}
        for segment in enriched_segments:
            key = (segment.start_stop.pure_id, segment.end_stop.pure_id)
            if key not in segment_delays:
                segment_delays[key] = []
            segment_delays[key].append(segment.average_delay)
        
        # Average delays for segments that appear multiple times
        for key in segment_delays:
            segment_delays[key] = np.mean(segment_delays[key])
        
        # Group routes by delay level for layering
        delay_groups = {
            'no_delay': folium.FeatureGroup(name="🟢 Időben (<2p)", show=True),
            'minor_delay': folium.FeatureGroup(name="🟡 Kis késés (2-10p)", show=True),
            'moderate_delay': folium.FeatureGroup(name="🟠 Közepes (10-20p)", show=True),
            'severe_delay': folium.FeatureGroup(name="🔴 Súlyos (20p+)", show=True),
        ...
        for route in hungary_routes:
            for pattern in route.patterns:
                ...
                # Calculate average delay for this route pattern
                pattern_delays = []
                for i in range(len(pattern.stops) - 1):
                    ...
                    if segment_key in segment_delays:
                        pattern_delays.append(segment_delays[segment_key])
                
                # Determine delay category and color
                if pattern_delays:
                    avg_delay = np.mean(pattern_delays)
                    delay_color = self.get_delay_color(avg_delay)
                    delay_category = self.get_delay_category(avg_delay)
                ...
                folium.PolyLine(
                    ...
                    popup=folium.Popup(
                        f"""
                        <div style="font-family: 'Segoe UI', Arial, sans-serif; min-width: 300px; background: white; padding: 20px; border-radius: 6px; border-left: 4px solid {delay_color};">
                            ...
                            <div style="background: #f8f9fa; padding: 10px; border-radius: 4px; margin: 10px 0;">
                                ...
                                <p style="margin: 4px 0; color: {delay_color}; font-size: 14px; font-weight: 600;">
                                    ⏱️ <strong>{delay_category}</strong> ({avg_delay:.1f} perc átlag)
                                </p>
                                <p style="margin: 4px 0; color: #666; font-size: 13px;">
                                    📊 <strong>{len(pattern_delays)} szegmens</strong> késési adattal
                                </p>
                            </div>
                    ...
                    tooltip=f"🚂 {route.short_name} - {delay_category} ({avg_delay:.1f}p)"
                ).add_to(target_group)
        ...
        print(f"✅ Added {total_patterns} delay-aware route patterns")
    ...
    def create_delay_aware_map(self, output_file="maps/delay_aware_train_map.html"):
        ...
        # Add stations
        self.add_delay_stations(map_obj, routes)
        
        # Legend removed per user request
        # self.create_delay_legend(map_obj)
...
def generate_delay_map_html(bulk_loader, route_data_dir="../../map_v2/all_rail_data", bulk_data_list=None):
    """
