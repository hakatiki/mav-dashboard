...
# Best-effort import that works both as a script and as a package module
try:
    from analytics.mav_analytics_library import MAVAnalytics, run_mav_analysis_for_date
...
def get_available_dates():
    """Get available dates from GCS dynamically"""
    try:
        ...
        # Fetch available dates from GCS
        available_dates = bulk_loader.get_available_dates_from_gcs()
        return available_dates
    except Exception as e:
        print(f"⚠️ Failed to fetch dates from GCS: {e}")
        print("📅 Using fallback dates...")
        # Fallback to hardcoded dates if GCS fails
        return [
            "2025-07-17", 
            "2025-07-18",
            "2025-07-19",
            ...
            "2025-07-29",
            "2025-07-30",
            "2025-08-06"  # Added the missing date
        ]
...
def main():
    ...
    if args.dates:
        ...
        # Validate dates
        invalid_dates = [d for d in dates_to_process if d not in available_dates]
        if invalid_dates:
